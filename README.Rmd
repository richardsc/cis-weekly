...
Both zip-collect.R and gpkg-collect.R are sufficiently lazy such that they do not decompress/load anything that has already been converted to a gpkg. The result is a folder gpkg/ that contains easy-to-read (i.e., `sf::read_sf()`) .gpkg files and a highly queryable attribute table (read using `arrow::read_parquet()`).

All of the geometry files are .gitignored because they don't fit on GitHub, but you can use the provided attrs.parquet and/or gpkg/meta.csv and/or zip/meta.csv to download a subset of the data (urls for each data source are provided in zip/meta.csv).
